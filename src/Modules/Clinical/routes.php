...
use App\Modules\Clinical\Controllers\PeriodontalChartController;
...
$router->group(['middleware' => [AuthMiddleware::class, LocationMiddleware::class]], function (Router $router) {
    ...
    // =========================================================================
    // CLINICAL CARE - Ruta raíz
    // =========================================================================

    // Redirigir /clinical a /agenda
    $router->get('/clinical', function() {
        return \App\Core\Response::redirect('/agenda');
    })->name('clinical.index');
    ...
    // Get periodontal chart for patient (AJAX)
    $router->get('/api/clinical/patients/{patientId}/periodontal-chart', [PeriodontalChartController::class, 'getChart'])
        ->name('api.clinical.periodontal-chart.get')
        ->middleware(PermissionMiddleware::class)
        ->permission('clinical.records.view');

    // Save periodontal chart (AJAX)
    $router->post('/api/clinical/patients/{patientId}/periodontal-chart', [PeriodontalChartController::class, 'saveChart'])
        ->name('api.clinical.periodontal-chart.save')
        ->middleware([PermissionMiddleware::class, CsrfMiddleware::class])
        ->permission('clinical.records.edit');

    // Get periodontal chart history (AJAX)
    $router->get('/api/clinical/patients/{patientId}/periodontal-history', [PeriodontalChartController::class, 'getHistory'])
        ->name('api.clinical.periodontal-chart.history')
        ->middleware(PermissionMiddleware::class)
        ->permission('clinical.records.view');

    // Compare periodontal charts (AJAX)
    $router->get('/api/clinical/patients/{patientId}/periodontal-compare', [PeriodontalChartController::class, 'compareCharts'])
        ->name('api.clinical.periodontal-chart.compare')
        ->middleware(PermissionMiddleware::class)
        ->permission('clinical.records.view');

    // Get periodontal chart summary (AJAX)
    $router->get('/api/clinical/patients/{patientId}/periodontal-summary', [PeriodontalChartController::class, 'getSummary'])
        ->name('api.clinical.periodontal-chart.summary')
        ->middleware(PermissionMiddleware::class)
        ->permission('clinical.records.view');

    // Export periodontal chart as PDF (AJAX)
    $router->get('/api/clinical/patients/{patientId}/periodontal-export', [PeriodontalChartController::class, 'exportPdf'])
        ->name('api.clinical.periodontal-chart.export')
        ->middleware(PermissionMiddleware::class)
        ->permission('clinical.records.view');
